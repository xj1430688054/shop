					...
					<s:iterator var="order" value="pageBean.list">
						...
						<s:iterator var="orderItem" value="#order.orderItems">
							<tr>
								...
								<td width="140">
									<span class="subtotal">￥<s:property
											value="#orderItem.subtotal" />
									</span>
								</td>
								<td>
								<%-- 	<a href="${ pageContext.request.contextPath }/assess_pageAssess.action" class="access">评价</a> --%>
								<s:if test="#order.state == 4">
									<s:if test="#order.status == 0">
										<a href="${ pageContext.request.contextPath }/assess_pageAssess.action?pid=<s:property value="#orderItem.product.pid" />&oid=<s:property value="#order.oid" />" class="assess">评价</a>
									</s:if>
									<s:if test="#order.status == 1">
										<a href="${ pageContext.request.contextPath }/assess_saveAssess.action" class="access">已评价</a>
									</s:if>
								</s:if>
								 <s:if test="#order.state == 3">
									请完成确认收获后评价
